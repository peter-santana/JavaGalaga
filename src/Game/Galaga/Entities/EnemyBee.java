...
public class EnemyBee extends BaseEntity {
    ...
    int timeAlive=0;
    long LastShotTime;
    //added a cool down for BeeLaser = AttackCoolDown
    int AttackCoolDown = random.nextInt(100)+55;
    boolean[][] Beegrid = new boolean[4][8];
    ...
    private void spawn() {
        ...
        switch (spawnPos){
            ...
            case 2://right
                ...
                y = random.nextInt(handler.getHeight()-handler.getHeight()/8);
                break;
                //Deleted beespawn from the downward position
    ...
    public void tick() {
        ...
        if (justSpawned){
            ...
            if (!centered && Point.distance(x,y,handler.getWidth()/2,handler.getHeight()/2)>speed){//reach center of screen
                ...
                if (timeAlive>=60*60*2){
                	...
                	//Changed this because of bug with score
                    remove = true;
                }
        	...
        	//if positioned, the attackCoolDown begins to count
        	AttackCoolDown --;
        ...
        //BeeLaser getter       
        if(AttackCoolDown<=0) {
        	handler.getGalagaState().entityManager.entities.add(new BeeLaser(x, y, width, height, Images.galagaBeeLaser, handler, handler.getGalagaState().entityManager, 1, 2));
        	AttackCoolDown = random.nextInt(100)+55;
    ...
    public void render(Graphics g) {
        //((Graphics2D)g).draw(new Rectangle(formationX,formationY,32,32)); edited this for the aesthetics
